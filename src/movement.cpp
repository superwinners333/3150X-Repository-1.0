...
void intakeDown(void) {
  Pistake.set(true);
  Pistake2.set(true);
}

void intakeNeutral(void) {
  Pistake.set(false);
  Pistake2.set(true);
}
...
void leverHalf(int speed) {
  ...
  while (levering == 1) {
    if (upwards) {
      if (liftUp) maxLeverAngle = 50;
      else maxLeverAngle = 44;
      RunIndex(100);
      lock.set(true);
      if (levertracker.position(degrees) < maxLeverAngle) RunLever(fabs(speed)); // leverspeed
      else {
    ...
    if (autolevertime.value() > 0.9) { // time before assuming the lever has stalled and exiting
      levering=0;
      RunLever(0);
      RunIndex(0);
    }
...
void driveToPoint(PIDDataSet KVals, double target_x, double target_y, double max_speed, double final_decel_speed, double timeout_ms, bool brake, double decel_distance, double Matchload_distance, double depoly_distance) {
    ...
    // count += 1;
    IVal = 0;
    // std::cout<<count<< "st movement "<<"X: "<<CPos.x<<" Y: "<<CPos.y<<std::endl;
    if (brake) {
        BStop();
        wait(100, msec);
    }
...
void driveToPointantiOrbit(PIDDataSet KVals, double target_x, double target_y, double max_speed, double final_decel_speed, double timeout_ms, bool brake, double decel_distance, double Matchload_distance, double depoly_distance) {
    Brain.Timer.reset();
    double distance_threshold = 0.1;
    double settling_zone = 1;
    double robot_radius = 2.41;
...
void dp1(PIDDataSet KVals, double target_x, double target_y, double max_speed,
                  double final_decel_speed, double timeout_ms, bool brake, double decel_distance, double Matchload_distance, double depoly_distance) {

...
void high_middle_wing() { // NEGATIVE TURNS TO THE LEFT
    ...
    NeutralScore();
    wait(200,msec);
    MoveEncoderPID(TestPara, -90, 10, 0.2, -178, false); // moves forward
    TurnMaxTimePID(TurnPara, -135, 0.4, false); // turns to face middle
    MoveEncoderPID(TestPara, 100, 32, 0.3, -136, false); // moves back into middle goal
    ...
    Wings.set(false);
    MoveEncoderPID(TestPara, 100, 12, 0.6, 179, false); // backs up to wing
    wait(200,msec);
    Move(-30,0);
    wait(100,msec);

...
void solo_awp() { // NEGATIVE TURNS TO THE LEFT
    ...
    MoveEncoderPID(TurnPara, -70, 19.8 , 0.3, 0,true); // drives to mathcloader
    Scrapper.set(true);
    RunRoller(100);
    TurnMaxTimePID(TurnPara, 90, 0.2, true); // turns to matchloader
    MoveTimePID(TurnPara, 50, 1.15 , 0.3, 90,false); // move into matchloader
    ...
    TurnMaxTimePID(TurnPara, -157, 0.2, false); // turns left
    RunRoller(100);
    ...
    MoveEncoderPID(TurnPara, -40, 20.5, 0.4, -157,true); // gets 3 blocks
    TurnMaxTimePID(TurnPara, -184, 0.2, false); // turns to other 3 blocks
    MoveEncoderPID(TestPara, -100, 24, 0.3, -184,false); // move to other side
    MoveEncoderPID(TestPara, -40, 16, 0.3, -184,true); // pick up other 3 balls
    Scrapper.set(true); // activates scraper
    TurnMaxTimePID(TurnPara, 135, 0.2, true); // turns to middle goal
    RunRoller(0); // stops intake
    MoveEncoderPID(TurnPara, 60, 10.6, 0.3, 135,true); // move to middle goal
    ...
    RunTopRoller(-15);
    MoveEncoderPID(TurnPara, -80, 37, 0.3, 135, true); // moves to between matchload tube and long goal
